...
function getCriticalViolations(violations: any[]) {
  ...
  return violations.filter(
    (v) =>
      v.impact === "critical" && !v.tags?.includes("best-practice") && !knownIssues.includes(v.id)
  );
}
...
test.describe("Accessibility - Automated Audits", () => {
  ...
  test("should pass axe accessibility audit on contact page", async ({ page }) => {
    ...
    await page
      .waitForSelector("text=/Loading contact info/i", {
        state: "hidden",
        timeout: 10000,
      })
      .catch(() => {});
      ...
      if (await button.isVisible()) {
        ...
        const hasFocusIndicator =
          (focusStyle.outline &&
            ...
            focusStyle.outline !== "rgb(0, 0, 0) none 0px") ||
          (focusStyle.boxShadow && focusStyle.boxShadow !== "none");
...
test.describe("Accessibility - ARIA Attributes", () => {
  ...
  test("should have proper heading hierarchy", async ({ page }) => {
    ...
    for (let i = 1; i < headings.length; i++) {
      ...
      if (levelDiff > 1) {
        console.warn(
          `Heading hierarchy skips from h${headings[i - 1].level} to h${headings[i].level}`
        );
      }
    }
  });
...
test.describe("Accessibility - Skip Links", () => {
  test("should have skip to main content link", async ({ page }) => {
    ...
    if (focusedElement.text?.includes("skip")) {
      ...
      const isValidSkipLink =
        focusedElement.href || focusedElement.tagName === "BUTTON" || focusedElement.hasOnClick;
