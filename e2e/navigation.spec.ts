...
test.describe("Navigation - Desktop", () => {
  test.beforeEach(async ({ page, browserName }) => {
    ...
    test.skip(
      process.env.CI === "true" && browserName === "firefox",
      "Firefox WebGL issues cause DOM instability in CI"
    );
...
test.describe("Navigation - Mobile", () => {
  ...
  test("should show hamburger menu button", async ({ page }) => {
    ...
    const menuButton = page.getByRole("button", {
      name: /toggle navigation menu|menu|navigation/i,
    });
...
test.describe("Navigation - Scroll Behavior", () => {
  test("should change header appearance on scroll", async ({ page, browserName }) => {
    ...
    // Get initial styles
    const initialBg = await header.evaluate((el) => window.getComputedStyle(el).background);
    ...
    const scrolledBg = await header.evaluate((el) => window.getComputedStyle(el).background);
