/* ============================================
   ...
   Cyan / electric blue / navy palette. Real depth.

   Ported from design-reference/src/supernova.js to use ESM imports
   instead of window.THREE, and exposes initSupernova(mount) returning
   a cleanup function.
...
export function initSupernova(mount) {
  if (!mount) return () => {};
  ...
  function init() {
    ...
    eventCleanups.push(() => window.removeEventListener('pointermove', onPointerMove));
    ...
    eventCleanups.push(() => window.removeEventListener('scroll', updateScroll));
    updateScroll();
    ...
    const onVisibility = () => {
      running = !document.hidden && !disposed;
      if (running) rafId = requestAnimationFrame(frame);
    };
    document.addEventListener('visibilitychange', onVisibility);
    eventCleanups.push(() => document.removeEventListener('visibilitychange', onVisibility));

    // Track outer-scope resources for cleanup
    cleanupResources = () => {
      cancelAnimationFrame(rafId);
      running = false;
      eventCleanups.forEach((fn) => fn());
      // Dispose materials + geometries
      [starMat, debrisMat, coreMat, beamMat, flashMat].forEach(
        (m) => m && m.dispose && m.dispose(),
      );
      shells.forEach((s) => {
        if (s.mesh && s.mesh.material && s.mesh.material.dispose) s.mesh.material.dispose();
        if (s.mesh && s.mesh.geometry && s.mesh.geometry.dispose) s.mesh.geometry.dispose();
      });
      nebulaGroup.children.forEach((mesh) => {
        if (mesh.material && mesh.material.dispose) mesh.material.dispose();
        if (mesh.geometry && mesh.geometry.dispose) mesh.geometry.dispose();
      });
      [starGeo, debrisGeo].forEach((g) => g && g.dispose && g.dispose());
      try { renderer.dispose(); } catch (_e) { /* noop */ }
      try { mount.removeChild(renderer.domElement); } catch (_e) { /* already gone */ }
    };
  }
