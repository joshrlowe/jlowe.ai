...
function imageSrc(image: string | ProjectImage): string {
...
export default function ProjectDetail({ project }: ProjectDetailProps) {
  ...
  const techStack = parseJsonField(project.techStack as JsonField, []) as Array<
    string | { name?: string }
  >;
  const features = parseJsonField(project.features as JsonField, []) as Array<
    string | { name?: string; title?: string }
  >;
  const challenges = parseJsonField(project.challenges as JsonField, []) as Array<
    string | { description?: string; name?: string }
  >;
  const links = parseJsonField(project.links as JsonField, {}) as ProjectLink;
  const papers = parseJsonField(project.papers as JsonField, []) as ProjectPaper[];
