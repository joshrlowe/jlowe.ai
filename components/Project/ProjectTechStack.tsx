...
import type { ProjectTechStackShape } from "./types";

const techStackToString = (techStack: ProjectTechStackShape): string => {
  const stack: string[] = [];

  if (techStack.fullStackFramework) stack.push(`${techStack.fullStackFramework}`);
...
const namesToString = (objects: { name: string }[]): string => {
  return objects.map((obj) => obj.name).join(", ");
};

interface ProjectTechStackProps {
  techStack: ProjectTechStackShape;
}
