...
const day = (date: string, count: number, level = 0): ContributionDay => ({ date, count, level });

describe("calculateContributionStats", () => {
  it("returns zeros for empty input", () => {
    expect(calculateContributionStats([])).toEqual({ total: 0, bestDay: 0, currentStreak: 0 });
  });
  ...
  it("counts the current streak from the most recent date backwards", () => {
    const stats = calculateContributionStats([
      day("2026-05-01", 1),
      day("2026-05-02", 1),
      day("2026-05-03", 1), // most recent — 3 contiguous active days
    ]);
  ...
  it("ends the streak when the most recent gap appears", () => {
    const stats = calculateContributionStats([
      day("2026-05-01", 5),
      day("2026-05-02", 5),
      day("2026-05-03", 0), // most recent — streak is 0
    ]);
