 ...
 *
 * Ported from v1 `lib/rag/chunker.ts`. Walks `marked.lexer()` tokens, tracks
 * heading hierarchy, keeps code blocks intact, splits long paragraphs at
 * sentence boundaries. Targets 300-500 tokens per chunk with a 50-token
 * overlap between adjacent chunks.
 *
 * v2 corpus entries can be shorter than MIN_TOKENS (a FAQ blurb, a one-screen
 * project). Pass `{ emitShort: true }` so those still become a single chunk
 * instead of disappearing from the index. Default stays v1-compatible.
 */
...
export interface ChunkOptions {
  /** Emit a chunk even when it is below MIN_TOKENS. Used by the index builder. */
  emitShort?: boolean;
}
...
function flush(
  ...
  if (!emitShort && state.tokens < MIN_TOKENS) {
    state.parts = [];
    state.tokens = 0;
    return;
  }
...
export function chunkMarkdown(md: string, options: ChunkOptions = {}): Chunk[] {
  ...
  const emitShort = options.emitShort === true;
  ...
  for (const tok of tokens) {
    if (tok.type === "heading") {
      ...
      flush(state, out, false, emitShort);
    ...
    if (tok.type === "code") {
      if (state.tokens > 0 && state.tokens + tokTokens > HARD_MAX_TOKENS) {
        flush(state, out, true, emitShort);
      }
    ...
    if (
      state.tokens >= MIN_TOKENS &&
      state.tokens + tokTokens > TARGET_TOKENS_MAX
    ) {
      flush(state, out, true, emitShort);
    }
    state.parts.push(raw);
