...
| `origin/main`                         | `264a0e3` — still the v1 tree (`#155` on top of `#163` noindex + admin-secret split)            |
| Last-good Vercel                      | `https://jlowe-ai.vercel.app` — HTTP/2 200, `server: Vercel`, `x-robots-tag: noindex, nofollow` |
| Divergence                            | `v2` is **181** unique commits ahead of `main`; `main` is **79** unique commits ahead of `v2`   |
...
`v2` and `main` diverged on purpose. As of this writing: **181** commits
only on `v2`, **79** only on `main`. A default recursive merge would try to
...
Decisions already made in the handoff. Task D (twin PRs #160–#173) is
**code-complete and live**; remaining gaps are config (Cal.com username,
Langfuse SSM values, SES production access), not ports. The **retire** rows
are accepted losses of the collapse. Do not re-open them on the merge PR.
...
| **Chat funnel: sessions, intent, `book_meeting`, citations, Langfuse** | `pages/api/chat.ts` + `lib/chat/intent.ts` + `lib/chat/tools.ts` (`book_meeting` → Cal.com) + Prisma `ChatSession` / `ChatMessageRow` + `lib/observability/langfuse.ts`. Citations from retrieved chunks. v1 fires `void trace.flush()`    | Retrieval + framed SSE (#167/#168); DynamoDB sessions + HttpOnly cookie (#169); intent + `book_meeting` fail-closed without Cal.com (#170); Langfuse with `await trace.flush()` after `stream.end()`, keys in SSM (#172). Cal.com username empty; Langfuse SSM value `unset`. | **ported** (#167–#172). Config-dark: Cal.com + Langfuse keys.                                         |
| **Upstash rate limit**                                                 | `lib/utils/rateLimit.ts` (`@upstash/ratelimit` sliding window). **Fails open** if `UPSTASH_REDIS_REST_URL` / `_TOKEN` are unset                                                                                                            | Edge WAFv2 `rate-limit-api-chat` (1000 / 5 min / IP, BLOCK) and `rate-limit-api-contact`. Per-session limiter is DynamoDB 10/60s (#169), fail-open on store errors                                                                                                            | **retire**                                                                                            |
...
The **retire** rows are accepted losses of the collapse. Do not re-open
them on the merge PR. Golden evals (#173) live in CI (credential-free
recall@5); nightly live Titan needs `GHA_EVAL_ROLE_ARN` (not set).
